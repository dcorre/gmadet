...
def phot_calib(candidates_list, telescope, radius=3,
    ...
    for i, key in enumerate(
            candidates_list.group_by("OriginalIma").groups.keys):
        ...
        detected_sources["mag_calib"] = detected_sources["mag_inst"] - \
            deltaMagMedian
        detected_sources["mag_calib_err"] = np.sqrt(
            detected_sources["mag_inst_err"] ** 2 + deltaMagStd ** 2
        )
    ...
    mag_calib_col = Column(np.zeros(len(candidates_list)),
                           name="mag_calib")
    mag_calib_err_col = Column(np.zeros(len(candidates_list)),
                               name="mag_calib_err")
    magsys_col = Column(["None"] * len(candidates_list),
                        name="magsys")
    filter_cat_col = Column(["None"] * len(candidates_list),
                            name="filter_cat")
    filter_DB_col = Column(["NoFilterFound"] * len(candidates_list),
                           name="filter_DB")

    candidates_list.add_columns([mag_calib_col, mag_calib_err_col,
                                 magsys_col, filter_cat_col,
                                 filter_DB_col])

    for j, filename in enumerate(filename_list):
        #  Compute calibrated magnitudes for transient candidates only
        mask = candidates_list["OriginalIma"] == filename
        candidates_list["mag_calib"][mask] = (
            candidates_list["mag_inst"][mask] - deltaMagMedianlist[j]
        )
        # Quadratic sum of statistics and calibration errors.
        candidates_list["mag_calib_err"][mask] = np.sqrt(
            candidates_list["mag_inst_err"][mask] ** 2 +
            deltaMagStdlist[j] ** 2
        )
    ...
    candidates_list.write(
        folder + fname2 + "_transient_candidates.dat",
        format="ascii.commented_header",
        overwrite=True,
    )
