...
def getCandPos(path, pattern=".alldetections"):
    ...
    # Otherwise create it
    try:
        table = ascii.read(os.path.join(path, "candidates_list.dat"))
    except BaseException:
        print("Can't read", os.path.join(path, "candidates_list.dat"))
        resfiles = glob.glob(
            os.path.join(path, "**", "*%s*" % pattern),
            recursive=True)
        filelist = []
        origfilelist = []
        Xpos_list = []
        ...
        table.write(
            os.path.join(path, "candidates_list.dat"),
...
def crossmatch_detections(path, candidates_list, radius=1, getFilter=False):
    ...
    # Otherwise create it
    try:
        sim_list = ascii.read(os.path.join(path, "crossmatch.dat"))
    except BaseException:
...
def makestats(path, radius=2):
    ...
    for band in bands:
        ...
        plt.grid(True)
        plt.legend()
        plt.savefig(os.path.join(path, "CheckSim/sim_mag_distrib_%s.png" % band[0]))
